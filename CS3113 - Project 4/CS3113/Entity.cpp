...
void Entity::update(float deltaTime, Entity *player, Map *map, 
    Entity *collidableEntities, int collisionCheckCount, Entity *enemies[], int enemyCount)
{
...
bool Entity::isCollidingWithEnemy(Entity* enemies[], int enemyCount)
{
    if (mEntityType != PLAYER) return false;
    
    for (int i = 0; i < enemyCount; i++)
    {
        if (enemies[i] != nullptr && isColliding(enemies[i]))
        {
            return true;
        }
    }
