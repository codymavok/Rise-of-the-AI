...
void Level1::panCamera(Camera2D *camera, const Vector2 *targetPosition)
{
    ...
    if (!mPlayerHit)
    {
        if (!mPlayerHit && mGameState.vincent->isCollidingWithEnemy(mEnemies, 1))
        {
            ...
            if (mGameState.lives != nullptr)
            {
                ...
                {
                    mGameState.nextSceneID = 1;  // Restart level
                }
                else
                {
