...
unsigned int Map::getTileAtPosition(Vector2 position) const
{
     if (position.x < mLeftBoundary || position.x > mRightBoundary ||
        position.y < mTopBoundary || position.y > mBottomBoundary)
        return 0;

    int tileX = (int)((position.x - mLeftBoundary) / mTileSize);
    int tileY = (int)((position.y - mTopBoundary) / mTileSize);

    if (tileX < 0 || tileX >= mMapColumns ||
        tileY < 0 || tileY >= mMapRows)
        return 0;

    return mLevelData[tileY * mMapColumns + tileX];
}

// unsigned int Map::getTileIndexAtPosition(Vector2 position) const // Need this to check if the tile at that position is the one user steps on to get to next level
// {
//     int index = getTileAtPosition(position);
